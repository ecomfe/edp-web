    ...
    app.post('/ls', function (request, response) {
        ...
        if (!dir || !fs.statSync(dir).isDirectory()) {
            ...
            files.forEach(function (file) {
                if (file.type === 'file') {
                    file.mime = util.mimeLookup(path.join(dir, file.name));
                }
            });
...
exports.routePreviewInfo = function (app) {
    app.post('/preview-info', function (request, response) {
        ...
        if (!fs.existsSync(file)) {
            ...
            var mimeType = util.mimeLookup(file);
            info.type = mimeType;
 ...
 * @param {express} app express服务实例
 */
exports.routeFileRemove = function (app) {
    app.post('/file-rm', function (request, response) {
        var file = request.body.file;
        var info = {};
